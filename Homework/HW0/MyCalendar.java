/* Modify a program that displays the calendar for the current month.
 *  You can use the Prior and Next buttons to show the calendar of the previous
 *   and next month. Display the dates in the current month in various color and 
 *   display the dates in the previous month and next month in different color
...
public class MyCalendar extends Application {
  ...
  public void start(Stage primaryStage) {
    ...
    String css1 = MyCalendar.class.getResource("MyStyle.css").toExternalForm();
...
class CalendarPane extends BorderPane {
  ...
  public void showDays() {
    ...
    for (int i = 0; i < startingDayOfMonth - 1; i++) {
      lblDay[i + 7].setTextFill(Color.ORCHID);
      lblDay[i + 7].setText(daysInPrecedingMonth - startingDayOfMonth + 2 + i + "");
    }
    ...
    int daysInCurrentMonth = calendar.getActualMaximum(
      ...
      lblDay[i - 2 + startingDayOfMonth + 7].setTextFill(Color.BLUEVIOLET);
      lblDay[i - 2 + startingDayOfMonth + 7].setText(i + "");
      
     }
 
     // got confused on how to make each month a different color once you press the next or previous button
    
    // Fill the calendar with the days after this month
    int j = 1;
    for (int i = daysInCurrentMonth - 1 + startingDayOfMonth + 7;
            i < 49; i++) {
      lblDay[i].setTextFill(Color.AZURE);
      lblDay[i].setText(j++ + "");
    }
  }
